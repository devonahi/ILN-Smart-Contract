...
function warnIfHardcodedSecret(secret) {
    ...
    if (knownExamples.has(secret)) {
        console.warn("[KeypairSigner] WARNING: You appear to be using a well-known example " +
            "secret key. Never use example keys in production.");
        return;
    }
    // Generic warning: secret supplied as a string literal rather than via env
    console.warn("[KeypairSigner] WARNING: Passing a secret key as a plain string is " +
...
class KeypairSigner {
    ...
    constructor(keypairOrSecret) {
        if (typeof keypairOrSecret === "string") {
            ...
            this._keypair = stellar_sdk_1.Keypair.fromSecret(keypairOrSecret);
        }
        else {
    ...
    static fromEnv(envVar) {
        ...
        if (!secret) {
            throw new Error(`KeypairSigner.fromEnv: environment variable "${envVar}" is not set`);
        }
        // fromEnv is the recommended pattern — no warning needed
        const kp = stellar_sdk_1.Keypair.fromSecret(secret);
